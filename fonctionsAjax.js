...
function actualiserPage() {
    ...
    if (objet_xml_http_request.readyState == 4) {
        ...
        if (objet_xml_http_request.status == 200) {
            ...
            resultatJson = objet_xml_http_request.responseText;

            console.log(resultatJson);

            var objet_json = JSON.parse(resultatJson);

            var gainTxt = objet_json.joueur.gain;
            var niveauJoueurTxt = objet_json.joueur.niveau;

            var niveau = niveauJoueurTxt;
            var gainAleatoire = parseInt(gainTxt);
